...
namespace GimApp.Clases
{
    class Ejercicios
    {
        public Ejercicios(string nombreEjercicio, int series, int repeticiones, Peso peso, string maquinaria, string grupoMuscular)
        {
            ...
            //Definir el path
            Path = $@"C:\Users\Linux-1\source\repos\GimApp\Guardado\{Nombre}.txt";

            while (File.Exists(Path))
            {
                int contador = 1;
                Path = $@"C:\Users\Linux-1\source\repos\GimApp\Guardado\{Nombre}({contador}).txt";
                contador++;
            }
        ...
        public void EditarEjercicio(string nombreEjercicio, int series, int repeticiones, Peso peso, string maquinaria, string grupoMuscular)
        {
            File.Delete(Path);
